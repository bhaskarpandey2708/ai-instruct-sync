...
export function main(input) {
  ...
  const slots = findSlots(dayStart, dayEnd, (Array.isArray(input.busy) ? input.busy : []), input.durationMin || 30);
...
export function findSlots(dayStart, dayEnd, busy, durationMin = 30) {
  ...
  while (t + step <= dayEnd) {
    const end = t + step;
    const conflict = (Array.isArray(busy) ? busy : []).some((b) => !(end <= b.start || t >= b.end));
