...
def _stories() -> list[ProductStory]:
    ...
    return [
        ...
        ProductStory(
            ...
            tagline="Keep agent skills in sync across Claude, Cursor, and friends",
            problem_kicker="SKILL DRIFT",
            problem_title="The same skill, four different copies",
            problem_body="You update a skill in one tool and the others keep running last month's version.",
            use_case="Teams that maintain shared agent skills across tools",
            setup_cmd="npx skill-sync status",
            term_title="zsh — skill-sync",
            badge="drift detected",
            capture=["node", "demo/run-demo.mjs"],
            fallback_lines=[
                "$ npx skill-sync status",
                "",
                "=== DEMO P06 skill-sync ===",
                "source   claude-skills/review.md",
                "targets  cursor · copilot · windsurf",
                "",
                "  WARN cursor   content hash differs (stale)",
                "  OK   copilot  in sync",
                "  WARN windsurf missing skill file",
                "",
                "Run: skill-sync apply --from claude",
            ],
            highlight=["WARN", "stale", "missing"],
            cta_cmd="npx skill-sync apply --from claude --dry-run",
        ),
        ProductStory(
